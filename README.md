...
Maintained by a SOC Analyst actively researching in Detection and Response for AI systems.

This is not vendor content. Everything here is written from a SOC perspective, with detection and response as the primary lens.
...
SIEM and EDR tooling built for human behaviour patterns cannot reliably detect an agent doing exactly what it was designed to do, under an attacker's direction. This repository is a working effort to bridge that gap. Every note is written with a detection or response angle in mind. The goal is to be useful to a Detection and Response engineer researching today.
